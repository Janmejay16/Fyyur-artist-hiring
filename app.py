...
def format_datetime(value):
  ...
  if value[5:7] == "01":
    ...
    month = "December"
  formatted_string = value[0:4] + " " + month + " " + value[8:10] + " at " + value[11:19] 
...
def check_availability(show_time,days_available):
  date = str(show_time[8:10]) + " " + str(show_time[5:7]) + " " + str(show_time[0:4]) 
  days = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
  day = datetime.strptime(date,"%d %m %Y").weekday()
  weekday = days[day]
  if weekday in days_available:
    status = "Valid"
  else:
    status =  "Not Valid"
  return status
...
def index():
  ...
  artists = Artist.query.order_by(desc(Artist.id)).limit(10)
...
def create_venue_submission():
  try:
    ...
    form = VenueForm()
    add_venue = Venue(name=form.name.data, city=form.city.data, state=form.state.data, address=form.address.data,
...
def show_artist(artist_id):
  ...
  data={
    ...
    "days_available" : artist.days_available,
    "past_shows": past_shows,
    "upcoming_shows": upcoming_shows,
    "past_shows_count": len(past_shows),
    "upcoming_shows_count": len(upcoming_shows)
...
def edit_artist_submission(artist_id):
  try:
    ...
    artist.days_available = form.days_available
...
def create_artist_submission():
  try:
    ...
    artist = Artist(name=form.name.data, city=form.city.data, state=form.city.data,
                    phone=form.phone.data, genres=form.genres.data, days_available = form.days_available.data,
...
def create_show_submission():  
  ...
  artist = Artist.query.get(artist_id)
  days_available = artist.days_available
  validity = check_availability(start_time, days_available)
  if validity=="Valid":
      db.session.add(show)
      db.session.commit()
      flash('Congrats! The artist is available for the entered date! Show was successfully listed!')
  elif validity=="Not Valid":
    flash("Artist is not available on that date !")
  else:
    db.session.rollback()
